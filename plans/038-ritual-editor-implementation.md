...
1. Complete the actual Next route browser matrix: conflicts, copy/paste, task-boundary deletion, reader output and offline draft recovery. Source/visual switching, save and reload, and visual typing have been exercised. The local reader currently reports unavailable because this environment has no published bundle for the disposable ritual; its SQL bundle count is zero, and the local `.env.local` does not configure bundle object storage. The semantic reader artifact itself exists in SQL. Publication and reader delivery need an environment with object storage. The local browser also logged an existing app-shell `MyAppBar` hydration mismatch unrelated to the editor.
